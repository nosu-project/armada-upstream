 ...
 * each 20s in useStreamAuth, subscription/wire rebuilds each 60s/2min).
...
function makeCommunity(n: number, heldRootCount: number): Community {
...
/** Mirror of useStreamAuth.ts `communityCoreKeys` (not exported). */
function coreKeys(community: Community): GroupKey[] {
  const keys: GroupKey[] = [];
...
/**
 * One full useRegisterAllStreamKeys pass (the 20-second poll), minus the
 ...
 */
function registerAllPass(communities: Community[], folds: FoldedControl[]): number {
  let derived = 0;
...
describe("derivation cost (perf guard)", () => {
  ...
  it("the 20-second useRegisterAllStreamKeys poll is near-free after the first pass", { timeout: 60_000 }, () => {
