 ...
 *
 * Deliberately narrower than a "run this batch atomically" interface (see
 * `@/lib/sqlite/driver`, which the old event store uses): the query planner
 * interleaves reads and writes inside a transaction — read the coordinate,
 * then supersede it — and issues `BEGIN IMMEDIATE` / `COMMIT` as ordinary
 * statements. A driver must therefore run statements in call order on a single
 * connection; it must NOT multiplex them across connections or reorder them.
 *
