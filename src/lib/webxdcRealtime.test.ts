...
describe("topic ids", () => {
  ...
  it("derives the fallback topic exactly as Rust does", () => {
    // The first input is the manifest NAME: Rust calls the parameter
    // `file_hash`, but both call sites pass the name.
    expect(deriveTopicId("Tic Tac Toe", "npub1abc", "evt-1")).toBe(TOPIC_TTT);
  });

  it("mints a fresh, valid topic every time", () => {
    const seen = new Set<string>();
    for (let i = 0; i < 50; i++) {
      const t = mintTopicId("hash", "senderhex");
...
describe("the URL-shared topic", () => {
  // Generated by Vector's own `derive_url_topic_id`.
  const SAMPLE = "QOLP2F6LAQZYTDGUKNGHFGESA5Q3P2HTWCTBZHBOQMLZ2DYHQDPQ";

  it("matches Vector byte for byte", () => {
    expect(deriveUrlTopicId("https://x.org/app.xdc", "msg1")).toBe(SAMPLE);
    expect(isTopicId(SAMPLE)).toBe(true);
  });

  it("anchors on the message, not the bytes", () => {
    // Two people who tap the same card hours apart share a session; a re-share
    // is a new message and therefore a new game, not a seat at the old one.
    const a = deriveUrlTopicId("https://x.org/app.xdc", "m1");
    expect(deriveUrlTopicId("https://x.org/app.xdc", "m1")).toBe(a);
    expect(deriveUrlTopicId("https://x.org/app.xdc", "m2")).not.toBe(a);
    expect(deriveUrlTopicId("https://x.org/other.xdc", "m1")).not.toBe(a);
  });

  it("is not the uploaded-file derivation", () => {
