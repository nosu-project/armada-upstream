...
import { KIND_COMMENT, KIND_DELETE, KIND_MESSAGE, KIND_TIMER_NOTICE } from "@/concord-v2/lib/kinds";
...
vi.mock("@/concord-v2/lib/rumorStore", () => ({ writeRumors: vi.fn(async () => true) }));
...
describe("publishTimerNotices (CORD-08 §4)", () => {
  ...
  function makeChannel(byte: number): ChannelV2 {
  ...
  it("posts one sealed notice per channel, and the notice never expires", async () => {
    ...
    const community = { idHex: "ab".repeat(32), relays: ["wss://a.example", "wss://b.example"] } as CommunityV2;
    ...
    const { writeRumors } = await import("@/concord-v2/lib/rumorStore");
