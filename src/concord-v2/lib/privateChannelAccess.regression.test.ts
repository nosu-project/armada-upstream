 ...
 *
 *   - `InviteDialog2.tsx:70` sends a Direct Invite with no channel narrowing;
 *   - `useInvites2.ts:336` mints a public link the same way;
 *   - `useRekey2.ts:904` re-publishes those links after a revoke rotation;
 *   - `useCommunityActions2.ts:423` mints a channel's access Role at
...
describe("CORD-03 §1 — a Private Channel is readable only by granted role-holders", () => {
  it("a Direct Invite must not carry a channel key the recipient holds no scoped Role for", () => {
    // Exactly what `InviteDialog2.tsx:70` -> `buildBundle` computes for a
  ...
  it("a public link bundle must not carry a role-gated channel key at all", () => {
    // `useInvites2.ts:336` mints with no narrowing either. A link has no
...
describe("CORD-06 §1 — a rekey severs the removed member", () => {
  it("the post-rotation link refresh must not re-vend the channel it just rotated", () => {
    // `useChannelRekey2` rotates #secret to epoch 5 to cut MEMBER, then
    // `useRekey2.ts:904` calls `refreshInviteBundlesFor`, which rebuilds every
  ...
  it("PASSES, and shows why the fix belongs in the refresh and not in the floor", () => {
    // The cut is recorded AT the excluding epoch (`useRekey2.ts:782`), and the
