...
vi.mock("@/concord-v2/lib/auditLog", () => ({ suspiciousActivity: () => h.detected }));
vi.mock("@/concord-v2/lib/control", () => ({ openControlEditions: () => [] }));
vi.mock("@/concord-v2/hooks/useControlPlane2", () => ({
  useControlEvents2: () => ({ data: [] as unknown[] }),
}));
vi.mock("@/hooks/useCurrentUser", () => ({ useCurrentUser: () => ({ user: { pubkey: ME } }) }));

const { useSuspiciousActivity2 } = await import("@/concord-v2/hooks/useSuspiciousActivity2");
...
const community = { idHex: "cc".repeat(32), id: new Uint8Array(32), relays: [] } as unknown as CommunityV2;
const folded = { roster, ownerHex: OWNER, banned: new Set<string>() } as unknown as FoldedControl;

const render = () => renderHook(() => useSuspiciousActivity2(community, folded));
...
describe("useSuspiciousActivity2 gating", () => {
