 ...
 * memos (useTransport2.ts). After the fold, the page runs ~10 more O(N) passes
 * over the whole decoded set on every switch — message adaptation
 * (useTransport2.ts:98), thread bucketing (:126), reaction/zap/poll tallies
 * (:178/:259/:331), rotation dividers (:459), and a final merge+sort (:447).
 * None are windowed to the rendered slice.
 *
 * This mirrors useTransport2.test.tsx's harness (mock useChannel2 to feed a
...
function renderTransport() {
  return renderHook(() => useTransport2(community, channel, true, false), {
    wrapper: ({ children }) => (
...
describe("useTransport2 channel-switch cost", () => {
  it(
    ...
    () => {
      ...
      console.log(
        `[perf] useTransport2 render: 100 msgs ${t100.toFixed(1)}ms · ` +
