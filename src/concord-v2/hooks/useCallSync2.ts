...
import { useCommunity2, useCommunityEntry2, useCommunityList2 } from "@/concord-v2/hooks/useCommunityList2";
import { useControlFold2 } from "@/concord-v2/hooks/useControlPlane2";
import { banVerdictPostdatesMembership, decideCallSync } from "@/concord-v2/lib/callSync";
 ...
 *   - HANGS UP when the folded Banlist names this membership, when the vault
 *     entry is gone (left, or the compliant ban self-removal ran), or when
 *     the channel left the live view (deleted / rotated key withheld).
 *
 ...
 * it down.
 */
export function useCallSync2(ctx: ConcordVoiceContext, onLeave: () => void): void {
  const { joinConcordCall } = useCall();
  const { user } = useCurrentUser();
  const { data: listData } = useCommunityList2();
  const community = useCommunity2(ctx.community.idHex);
  const entry = useCommunityEntry2(ctx.community.idHex);
  ...
  }, [ctx, listData, community, folded, channels, entry, user, onLeave, joinConcordCall]);
}
