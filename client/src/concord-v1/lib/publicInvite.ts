...
import { publicInviteKey, publicInviteLocator, publicInviteSigner } from "@/concord-v1/lib/derive";
...
/** A parsed invite fragment: the bootstrap relays and the token. */
 ...
 * Accepts the full URL or just the fragment after `#`, in the v2 binary format
 * or the legacy v1 JSON format. A v1 fragment is base64url(JSON) whose first
 * decoded byte is `{` (0x7B); the binary format's first byte is the version
 * (2). They never collide.
 */
export function parseInviteUrl(url: string): ParsedInviteUrl {
  ...
  if (raw[0] === URL_V2) return parseBinary(raw, TRUSTED_RELAYS, RELAY_DICTIONARY);
