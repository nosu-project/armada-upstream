...
import { signEdition } from "@/concord-v1/lib/edition";
import { communityMetadataOf } from "@/concord-v1/lib/metadata";
import { buildOwnerAttestationUnsigned, signOwnerAttestation } from "@/concord-v1/lib/owner";
import { adminRole } from "@/concord-v1/lib/roles";
...
describe("encrypted community image", () => {
  it("encrypts then decrypts back to the original bytes (integrity verified)", async () => {
    const { encryptImage, decryptImageToObjectURL } = await import("@/concord-v1/lib/communityImage");
