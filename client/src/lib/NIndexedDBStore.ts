...
// `authors`, `kinds`, single-letter tag filters (`#e`, `#p`, `#h`, `#z`, …),
// `since`/`until`, `limit`, `search`, and any combination thereof.
//
// This is what makes chat history survive a page refresh: a query like
// `{ kinds: [9], "#h": [groupId] }` (NIP-29), `{ kinds: [4], "#p": [me] }`
// (DMs), or `{ kinds: [3300], "#z": [pseudonyms] }` (Concord) resolves
// directly from the tag index below, so the timeline can be read back from
// IndexedDB without any per-conversation id-list bookkeeping.
//
...
export class NIndexedDBStore implements NStore {
  ...
  /**
   * The database connection, or `null` if IndexedDB is unavailable, in which
   * case every method silently degrades.
   */
  private readonly db: IDBPDatabase<EventsDB> | null;

  private constructor(
    db: IDBPDatabase<EventsDB> | null,
    indexTags: (event: NostrEvent) => string[][],
  ) {
    this.db = db;
   ...
   * method silently degrades. Bumping the schema version drops and recreates
   * the events store (the cache is a disposable mirror, never the source of
   * truth), so an old layout is simply discarded.
   */
  static async open(opts: NIndexedDBStoreOpts = {}): Promise<NIndexedDBStore> {
    const { name = 'ditto-events', version = 2, indexTags = NIndexedDBStore.indexTags } = opts;

    let db: IDBPDatabase<EventsDB> | null = null;
    try {
      db = await openDB<EventsDB>(name, version, {
        upgrade(database) {
          // Drop every existing store (any prior layout) and rebuild fresh.
  ...
  private async flushWrites(): Promise<void> {
    ...
    try {
      ...
      // batch up front, with all the index reads issued in parallel.
      ...
      // NIP-09: apply any deletion requests in this batch.
  ...
  private async resolveSupersession(
    ...
    const existingPerCoord = await Promise.all(
      entries.map(([sample]) =>
        store
          ...
          .getAll(
            IDBKeyRange.bound([sample.pubkey, sample.kind, 0], [sample.pubkey, sample.kind, Infinity]),
    ...
    entries.forEach((batchForCoord, i) => {
      ...
      const existing = existingPerCoord[i].filter(
        (other) => !NKinds.addressable(other.kind) || NIndexedDBStore.getDTag(other) === dTag,
      );
   ...
   * events it targets from the cache. A deletion request can only delete the
   * requester's own events. The kind 5 event itself is never deleted here —
   * clients may still need it to re-broadcast the deletion.
   */
  private async applyDeletions(store: EventsStore<'readwrite'>, events: NostrEvent[]): Promise<void> {
      ...
      for (const [name, value] of request.tags) {
        if (name === 'e') {
          ...
          for (const event of stored) {
            if (NKinds.addressable(event.kind) && NIndexedDBStore.getDTag(event) !== dTag) continue;
  ...
  private async queryFilter(
    ...
    for (const cursor of plan.cursors) {
      ...
      // Each cursor walks its prefix newest-first; stop early once we have the
      // limit so scan cost grows with the limit, not the cache size.
      let matched = 0;
  ...
  private static planScan(filter: ParsedFilter): ScanPlan {
    ...
    // 2. tags — pick the most selective tag filter (fewest values).
...
// plus an in-memory matcher. A TypeScript port of strfry's `NostrFilter`
// (src/filters.h): it pre-sorts/dedupes each value set, counts the "major"
// fields used by the query planner, and decides whether a scan can be satisfied
// by an index alone (`indexOnly`) or needs a post-filter re-check.
// ============================================================================
...
class ParsedFilter {
  ...
  /** True when this filter can never match anything (an empty array constraint). */
  readonly neverMatch: boolean;

  /** True when a single index scan fully satisfies the filter. */
  ...
  constructor(filter: NostrFilter) {
    ...
    this.indexOnly =
      !hasSearch && (numMajorFields <= 1 || (numMajorFields === 2 && !!this.authors && !!this.kinds));
  }
