 ...
 * paints immediately; once the live fold (recomputed from the freshly re-read
 * control events) is available it takes over and is written back for next time.
 *
...
export function usePersistedFold<T>(key: string | null, live: T | undefined): T | undefined {
  ...
  // Load the persisted snapshot once per key.
  useEffect(() => {
    if (!key) return;
    let cancelled = false;
