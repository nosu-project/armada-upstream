...
/**
 * Chat messages (kind 9) for a NIP-29 group, with a live subscription that
 * appends incoming messages into the query cache.
 *
  ...
  const query = useQuery<NostrEvent[]>({
    ...
    queryFn: async ({ signal }) => {
      const events = await nostr.relay(relayUrl!).query(
        [{ kinds: [KIND_GROUP_CHAT], "#h": [groupId!], limit: 200 }],
    ...
    (async () => {
      try {
        for await (const msg of nostr.relay(relayUrl).req(
          [{ kinds: [KIND_GROUP_CHAT], "#h": [groupId], since: Math.floor(Date.now() / 1000) - 5 }],
