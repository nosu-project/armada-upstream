 ...
 * kind-9 message that p-tags them, from ANY of the community's channels,
 * newest-first. Purely local: served from the decrypted rumor cache
 * (IndexedDB), which the wire keeps fed for every channel (no relay query).
 *
 * Modeled on {@link useConcord2Unread}: the scan lives in the shared TanStack
 * Query cache (keyed by pubkey + the channel set) and re-runs the moment the
 * wire bus announces a `c2:` change to any watched channel, with a light poll
 * as a backstop. Each row carries its own `channel` binding, so the view can
 * label which channel a mention came from.
 */
export function useConcord2Mentions(channels: ChannelV2[]): {
  ...
  const { data: mentions = [], isLoading } = useQuery<ChatMsg[]>({
    ...
    queryFn: async () => {
      const rumors = await queryMentionRumors(channelIds, pubkey!, { limit: MENTION_LIMIT });
