 ...
 *  (kept in sync with the chat `Lightbox`). */
const EASING = "cubic-bezier(0.25, 0.46, 0.45, 0.94)";
const DURATION = 280;

/** Fullscreen viewer for a single already-decrypted image URL.
 *
 * Escape / click closes; on touch, drag the image vertically (up or down) to
 * dismiss — past 15% of the viewport height commits (the content flies
 * off-screen and closes), otherwise it springs back. The backdrop fades with
 * drag distance. Same gesture as the chat `Lightbox` so banner/avatar previews
 * behave identically. Refs + direct DOM mutation (no state) keep the drag at
 * 60fps. */
export function ImageLightbox2({ src, onClose }: { src: string; onClose: () => void }) {
  ...
  onTouchMoveRef.current = (e: TouchEvent) => {
    ...
    // Only a vertical drag dismisses (a single image has no horizontal nav).
