 ...
 * web client's {@code ReplyPreview} does for an on-screen preview:
 *
 ...
 *     service resolves from its cache/DB, never the network); an author that
...
final class NotificationContent {
    ...
    private static String resolveMentions(String s, MentionResolver resolver) {
        ...
        while (m.find()) {
            ...
            String name = pubkey != null ? resolver.nameFor(pubkey) : null;
